...
function UserDetails() {
    ...
    useEffect(() => {
        userSub = UserService.getUserByUsername(username).subscribe((res: AjaxResponse<User>) => {
    ...
    const isConnectedWith = (): boolean => {
        return AuthenticationService
                .getCurrentUser()!.connections
                .some((u: User) => u.username === user?.username);
    }

    const connectWithUser = () => {
        if (user !== undefined)
            connectSub = UserService.connectWithUser(user)?.subscribe((res: AjaxResponse<HttpResponse>) => {
                console.log(res.response);
        ...
        <div>
            ...
            <Container mt='2rem'>
                <Flex direction='column'>
                    <Flex direction='row'>
                        <Image src='/logo192.png' boxSize='125px' border='solid 1px #2b6cb0' borderRadius='2xl' />
                    ...
                    <Flex mt='2rem' direction='row' alignSelf='center'>
                        {
                            isConnectedWith()
                                ? ''
                                : <Button type='button' onClick={() => connectWithUser()} ml='-6rem' mr='3rem' colorScheme='cyan'>
                                    Connect
                                  </Button>
                        }
            ...
            { alertOpen ? <Alert message={ alertMsg } open={ alertOpen } onModalClose={ closeAlert } /> : "" } 
        </div>
    )
}
